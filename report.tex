...
When fluid can conduct electricity with negligible resistivity, the fluid is 
tied to magnetic fields threading itself. We can combine Ohm's Law in ideal 
...
Considering short line element $d\mathbf{l}$ moving with fluid, we can express 
...
$d\mathbf{l}$ obey the same equation [\cite{Davidson2001}]. Therefore, the 
...
\end{align}
where $K$ is analogous to a spring constant \citep[see][]{Wiki:MRI}. The 
...
profile. We present a linear stability analysis of MRI for bounded case 
...
To investigate MRI, one can consider homogeneous incompressible fluid rotating 
...
$\hat{u}_r = u$, \cite{Acheson1973a} obtained following eigenvalue problem,
\begin{align}
...
\cite{Knobloch1992} shows exponentially growing instability is only possible 
